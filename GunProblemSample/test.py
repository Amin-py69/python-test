class Shooter:
    ...
    def set_gun_by_name(self, name: str) -> None:
        ...
        if name not in guns:
            raise Exception(f"The gun is not available: {name}")
    ...
    def shoot_to_target(self, target_x: int, target_y: int, target_distance: int, aim_x: int, aim_y: int) -> float:
        ...
        if target_distance > gun_distance[self.gun_name]:
            ...
            if gun_power[self.gun_name] * bullet_damage[gun_power[self.gun_name]] < 0:
                raise Exception("Invalid gun power or bullet damage.")

            return gun_power[self.gun_name] * bullet_damage[gun_power[self.gun_name]]
